...
/**
 * Custom node evaluator for recruits (BASE version).
 *
 * Keeps the vanilla WalkNodeEvaluator skeleton (collision / step-up / fall
 * logic) but adds explicit COST SHAPING for our concrete problems:
 *
 *   - fall depth: 2-3 small malus, 4-5 heavy malus, >5 blocked. Because we are
 *     async we can afford to scan downward every node.
 *   - water distance: nodes near water get a linear malus so recruits walk
 *     around water instead of into the border. Not a hard block (narrow bridge
 *     still works).
 *   - dead ends: nodes with very few walkable neighbours get a soft malus so
 *     the search prefers open corridors and avoids pockets / holes.
 *   - dirt path preference retained as a small bonus.
 *
 * The "lands exactly on the block" / "underground target" problems are mostly
 * fixed in AsyncPathfinder (Y-weighted heuristic + reach-based selection); the
 * shaping here makes sure the cheapest path is also a reachable one.
 *
 * All tunables are the constants at the top.
 */
public class WorkersNodeEvaluator extends NodeEvaluator {

    // ---- tuning constants -------------------------------------------------
    private static final int SOFT_FALL_MIN = 2;
    private static final float SOFT_FALL_MALUS = 2.0F;
    private static final int HARD_FALL_MIN = 4;
    ...
    private static final float PATH_BLOCK_BONUS = 2.0F;
    // -----------------------------------------------------------------------
    ...
    protected @NotNull Node getNode(int x, int y, int z) {
        ...
        // --- fall depth -----------------------------------------------------
        int fall = scanFallDepth(pos);
        if (fall > MAX_SAFE_FALL) {
            node.costMalus = -1.0F;
            node.type = BlockPathTypes.BLOCKED;
            return node;
        } else if (fall >= HARD_FALL_MIN) {
            node.costMalus += HARD_FALL_MALUS;
        } else if (fall >= SOFT_FALL_MIN) {
        ...
        if (exits <= DEAD_END_EXITS) {
            node.costMalus += DEAD_END_MALUS;
        }

        // Prefer walking on top of configured path blocks (defaults: dirt_path,
        // gravel, cobblestone; plus anything in the PreferredPathBlocks config).
        Block below = this.level.getBlockState(pos.below()).getBlock();
    ...
    /** how many blocks down until solid ground (0 = standing on ground). */
    private int scanFallDepth(BlockPos pos) {
        BlockPos.MutableBlockPos m = new BlockPos.MutableBlockPos();
        int depth = 0;
        for (int dy = 1; dy <= MAX_SAFE_FALL + 2; dy++) {
            m.set(pos.getX(), pos.getY() - dy, pos.getZ());
            BlockPathTypes t = getBlockPathTypeStatic(this.level, m);
            if (t != BlockPathTypes.OPEN && t != BlockPathTypes.WATER && t != BlockPathTypes.LAVA) {
                return depth;
            }
            depth++;
        }
    ...
    private int countWalkableNeighbours(BlockPos pos) {
        int exits = 0;
        BlockPos.MutableBlockPos m = new BlockPos.MutableBlockPos();
        for (Direction dir : Direction.Plane.HORIZONTAL) {
            m.set(pos.getX() + dir.getStepX(), pos.getY(), pos.getZ() + dir.getStepZ());
            BlockPathTypes type = getBlockPathTypeStatic(this.level, m);
            if (type != BlockPathTypes.BLOCKED
                    && type != BlockPathTypes.FENCE
                    && type != BlockPathTypes.LAVA
                    && this.mob.getPathfindingMalus(type) >= 0.0F) {
                exits++;
            }
        }
        return exits;
