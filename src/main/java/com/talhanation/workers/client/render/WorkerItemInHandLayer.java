...
@OnlyIn(Dist.CLIENT)
public class WorkerItemInHandLayer<T extends AbstractInventoryEntity, M extends EntityModel<T> & ArmedModel & HeadedModel> extends ItemInHandLayer<T, M> {
    private static final float X_ROT_MIN = (-(float)Math.PI / 6F);
    private static final float X_ROT_MAX = ((float)Math.PI / 2F);

    public WorkerItemInHandLayer(RenderLayerParent<T, M> p_174516_) {
        super(p_174516_);
    }
