...
@OnlyIn(Dist.CLIENT)
public class WorkerItemInHandLayer<T extends AbstractInventoryEntity, M extends EntityModel<T> & ArmedModel & HeadedModel>
        extends ItemInHandLayer<T, M> {
    static final float X_ROT_MIN = (-(float) Math.PI / 6F);
    static final float X_ROT_MAX = ((float) Math.PI / 2F);
    private final ItemInHandRenderer renderer;

    public WorkerItemInHandLayer(RenderLayerParent<T, M> p_174516_, ItemInHandRenderer renderer) {
        super(p_174516_, renderer);
        this.renderer = renderer;
    }

    protected void renderArmWithItem(LivingEntity p_174525_, ItemStack p_174526_,
            ItemTransforms.TransformType p_174527_, HumanoidArm p_174528_, PoseStack p_174529_,
            MultiBufferSource p_174530_, int p_174531_) {
